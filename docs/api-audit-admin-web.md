...
| 7 | A | Patrol Operations | GET | `/api/patrols` | List patrols (dashboard, `/patrols`, `/patrols/all`, `/patrols/history`) | `▲` `patrols.ts:28/55` | Query `{mine?, status?, forestId?}`; adapted via `patrolFromApi`; mock fallback in services |
...
| 9 | A | Observations | GET | `/api/incidents` | List observations/incidents | `▲` `incidents.ts:33/66` | Query `{mine?, status?, type?, from?, to?}`; reshaped via `observationFromApi`; mock fallback |
...
| 13 | A | GIS Intelligence | GET | `/api/gis/beats` | Beat boundaries as GeoJSON (props `Beat`, `Division`, `Range`, `Coverage_pct`) | `▲` `gis.ts:12` | `auth:false`; → `beatsFromGeoJson` (SVG viewBox 1000×700); mock `mapBeatsRaw` fallback; feeds `/gis` map, dashboard overview, zero-patrol derivation |
| 14 | A | GIS Intelligence | GET | `/api/gis/compartments` | Compartment boundaries as GeoJSON (props `COMP_NO`, `BEAT`, `AREA_HA`) | `▲` `gis.ts:50` | `auth:false`; → `compartmentsFromGeoJson`; shares `unionExtent` projection with beats so both layers align; `[]` fallback |
...
| 22 | B | Patrol Operations | GET | `/api/patrols/:id/points` | GPS trace points `{lat,lng,altitude?,speed?,t}` | `▲` `patrols.ts:114` | Prepared for route replay; replay screen currently uses mock/synth routes |
...
| 35 | B | Platform — Alerts | GET | `/api/alerts` | SOS / tamper / coverage alert feed | `✖` **no backend route found** | Web stub only; expects query `{since?, limit?}`; backend likely pushes via websocket — decide: build route or drop |
...
- **A — Live:** 19 endpoints (auth 5, patrols 2, observations 4, GIS 5 incl. web tile proxy, admin users 3)
- **B — Client-defined, unwired:** 20 endpoints (1 orphan: `/api/alerts` has no backend route)
- **C — Backend-only:** 9 endpoint groups (mobile / GIS import tooling)
- **Architecture rule:** UI talks only to `lib/services.ts`; every fetch uses `tryRemote` → on transport/401/404/≥500 falls back to `lib/mock/*`, so all A/B features still render without the backend (`isRetryableFailure`, api.ts:37)
